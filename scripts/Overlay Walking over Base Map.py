...
# -*- coding: utf-8 -*-
"""
Overlay Walkable Tiles Over Base Map (with Grid & Axes)
=======================================================

Author: Pham Duc Minh
Purpose
-------
Draws a walkability overlay on top of each base map image by:
  1) Rendering a tile-aligned grid and optional row/column axes labels
  2) Dim-masking non-walkable tiles (optional)
  3) Plotting a center dot on every walkable tile (with optional glow)

Normalization Across Scripts
----------------------------
This script adopts the same parameter and column naming conventions used
in your overlay/extraction tools so components can be mixed & matched:

- MAP/IMAGE KEY
  * PARAMS["MAP_ID_COLUMN"]  -> column holding the map key (e.g., "source")
- TILE COORDINATES
  * row/col  OR  x/y (pixels). Autodetected; forced names allowed via FORCED_COL_NAMES
- TILE GEOMETRY
  * DEFAULT_TILE_W/H and GRID_OFFSET_X/Y for lattice math when row/col are used
- WALKABILITY FLAG
  * PARAMS["MANUAL_OVERRIDE_COLUMN"] (autodetects "manual_override"/"label"/"class"...)
  * "walkable" includes {"air","walkable","passable","0","true","t","yes","y"}

I/O Conventions
---------------
- Base images resolved from:
    SOURCE_DIR / ( <map_id> + SOURCE_EXT )
  unless <map_id> is already an absolute/relative path to a file.
- All outputs go to OUT_DIR with per-map tags.

Sections
--------
1) Parameters (PARAMS): single source of truth for I/O, styling, geometry.
2) Column Autodetection: robust mapping to your CSV schema(s).
3) Helpers: path building, color helpers, sanitization.
4) Layers:
   - dim_non_walkable_layer(): dims everything except walkable tiles
   - dot_layers_for_map(): walkable center dots (+ optional glow)
   - grid_layer(): tile lattice lines
   - axes_layer(): row/column indices at cadence
5) Main: CSV -> group by map -> assemble layers -> write artifacts.
...
PARAMS: Dict[str, object] = {
    ...
    # Path to the master CSV file containing tile-level information
    # (e.g., tile position, type, passability, and manual override columns).
    # Usually exported from legend generation or extraction scripts.
    "CSV_PATH": "../../legend_all_tiles.csv",

    # Directory containing the base map image(s) that correspond to
    # each record in the CSV. Each “source” or “map_id” should match
    # a file name inside this folder.
    "SOURCE_DIR": "../sources",

    # Default extension for the map files. If a record’s map name
    # (e.g., “tloz1_1”) doesn’t include an extension, this string
    # (e.g., ".png") is appended automatically.
    "SOURCE_EXT": ".png",

    # Directory where the overlay images and all generated
    # intermediate layers (dots, glow, grid, etc.) will be saved.
    "OUT_DIR": "../../OUT",
...
def main():
    ...
    for sid, sdf in df.groupby(cols["map_id"]):
        ...
        # Build layers
        layer_dim  = dim_non_walkable_layer(sdf, cols, (W, H))
        layer_glow, layer_dots = dot_layers_for_map(sdf, cols, (W, H))
        layer_grid = grid_layer((W, H), tw, th, ox, oy)
        layer_axes = axes_layer((W, H), tw, th, ox, oy)

        # Compose: base -> dim -> grid -> glow -> dots -> axes
        comp = base.copy()
        comp.alpha_composite(layer_dim)
        comp.alpha_composite(layer_grid)
        comp.alpha_composite(layer_glow)
        comp.alpha_composite(layer_dots)
        comp.alpha_composite(layer_axes)

        # Persist artifacts
        tag = _sanitize(sid)
        out_dir = str(PARAMS["OUT_DIR"])
        base.copy().save(os.path.join(out_dir, f"{tag}_base.png"))
        layer_dim.save( os.path.join(out_dir, f"{tag}_dim.png"))
        layer_grid.save(os.path.join(out_dir, f"{tag}_grid.png"))
        layer_axes.save(os.path.join(out_dir, f"{tag}_axes.png"))
        layer_glow.save(os.path.join(out_dir, f"{tag}_walkable_glow.png"))
        layer_dots.save(os.path.join(out_dir, f"{tag}_walkable_dots.png"))
